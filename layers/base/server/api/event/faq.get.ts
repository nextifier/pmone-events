export default defineCachedEventHandler(
  async (event) => {
    ...
    try {
      ...
      // most recent event by start_date so FAQ still renders even when no event
      // is currently flagged is_active=true. Only when neither exists do we return
      // an empty payload so the component shows its empty state instead of a
      ...
      const faq = await $fetch(
        `${config.public.apiUrl}/api/public/projects/${username}/events/${eventSlug}/faqs`,
        {
          ...baseOpts,
          query: { locale },
        },

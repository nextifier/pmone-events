...
function updateResolvedColor() {
  if (!containerRef.value || !color.value) {
    resolvedColor.value = "rgb(0, 0, 0)";
    return;
  }
  const c = color.value.trim();
  // Resolve CSS variable from the container element
  if (c.startsWith("var(")) {
    const varName = c.slice(4, -1).trim();
    const raw = getComputedStyle(containerRef.value).getPropertyValue(varName).trim();
    // Resolve to a canvas-compatible format
    const ctx = document.createElement("canvas").getContext("2d")!;
    ctx.fillStyle = raw || "rgb(0, 0, 0)";
    resolvedColor.value = ctx.fillStyle;
    return;
  }
  const ctx = document.createElement("canvas").getContext("2d")!;
  ctx.fillStyle = c;
...
onMounted(() => {
  ...
  // Re-resolve color when color mode changes (dark/light toggle)

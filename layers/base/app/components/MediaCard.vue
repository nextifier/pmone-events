<template>
  ...
  >
    <span
      class="text-foreground text-2xl font-semibold tracking-tighter text-balance"
      >{{ item.title }}</span
    >
    <div class="flex w-full items-center justify-between gap-x-3">
      <div class="flex flex-col items-start gap-y-1">
        <span
          v-if="item.link"
          class="text-foreground line-clamp-1 text-sm font-semibold tracking-tight"
          >{{
      ...
      <nuxt-link
        ...
        class="bg-muted text-foreground hover:bg-border flex size-12 shrink-0 translate-x-2 items-center justify-center rounded-full transition active:scale-98"
        v-ripple
      >

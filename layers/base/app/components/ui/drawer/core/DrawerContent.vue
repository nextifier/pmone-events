...
import { useEmitAsProps, useForwardExpose } from "reka-ui"
import { useHideOthers } from "./shared/useHideOthers"
  ...
  <Presence :present="forceMount || rootContext.open.value">
    <DrawerContentImpl
      ...
      @pointer-down-outside="(e: any) => {
        ...
        // A press on the trigger belongs to the trigger. Without this the press
        // dismisses here on `pointerdown` and the trigger's own `click` fires a
        // moment later against an already-closed drawer — a toggling trigger
        // would reopen it and the drawer could never be closed from its own
        // button. The non-modal branch below has always had this guard.
        if (rootContext.triggerElement.value?.contains(e.target as HTMLElement))
          e.preventDefault()
      }"

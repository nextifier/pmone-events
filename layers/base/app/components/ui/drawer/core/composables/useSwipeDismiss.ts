...
/**
 * Base UI's opt-out, for subtrees running a gesture of their own — carousels,
 * sliders, sortables. `DEFAULT_IGNORE_SELECTOR` above is reka's own list of
 * interactive elements and applies to the mouse only: on touch a drag has to
 * start from anywhere on the panel, buttons and fields included.
 */
const SWIPE_IGNORE_SELECTOR = '[data-drawer-swipe-ignore],[data-base-ui-swipe-ignore],[data-swipe-ignore],input[type="range"],[role="slider"]'
...
export function useSwipeDismiss(options: UseSwipeDismissOptions) {
  ...
  function reset() {
    // A rubber-band pull never became a real swipe, so nothing else will
    // clear the offset it left on the element.
    const rubberBandEl = elementRef.value
    if (rubberBandEl?.hasAttribute('data-rubber-band')) {
      rubberBandEl.removeAttribute('data-rubber-band')
      clearCssVars(rubberBandEl)
    }
    setSwiping(false)
    ...
    // Announce the reset, do not just record it. reka cleared the number here
    // and never emitted it, so every consumer of `onProgress` — the backdrop,
    // the stacking of drawers behind this one — kept the value the finger left
    // behind, for the rest of the drawer's life and into its next open.
    if (swipeProgress !== 0) {
      swipeProgress = 0
      onProgress?.(0, { deltaX: 0, deltaY: 0, direction: intendedDirection })
    }
    swipeProgress = 0
    lastDragSample = null
  ...
  function processMove(el: HTMLElement, pos: { x: number, y: number }, time: number) {
    ...
    if (pendingSwipe && pendingSwipeStartPos) {
      ...
      if (!dir) {
        // Base UI still follows the finger here, damped, so a one-direction
        // drawer gives under a pull the wrong way instead of feeling dead.
        // `data-rubber-band` lets the CSS drop its transition for the frame.
        const idle = applyDirectionalDamping(dx, dy)
        dragOffset.value = { x: idle.x, y: idle.y }
        setCssVars(el, idle.x, idle.y)
        if (idle.x !== 0 || idle.y !== 0)
          el.setAttribute('data-rubber-band', '')
        return
      }
    ...
    if (currentEl) {
      ...
      const p = Math.min(1, Math.max(0, displacement / dim))
      if (p !== swipeProgress) {
        swipeProgress = p
  ...
  function finishSwipe(el: HTMLElement) {
    ...
    if (!releaseHandled) {
      ...
      const shouldDismiss = !cancelledSwipe
        && (displacement >= threshold || velInDirection > 0.5)
  ...
  function onPointerDown(e: PointerEvent) {
    ...
    if (target?.closest(DEFAULT_IGNORE_SELECTOR) || target?.closest(SWIPE_IGNORE_SELECTOR))
      return

...
const display = computed(() => value.value.toFixed(Math.min(stepDecimals.value, 4)) * 1);
...
function startDrag(e) {
  ...
  const onMove = (e2) => { lastX = e2.clientX; setFromX(e2.clientX, dragGrid.value); };
  const onUp = () => {
...
<template>
  <div class="tl-field-wrap">
    <div
      ...
      @dblclick="onDblClick"
    >
      <div class="tl-field-fill" :class="{ 'is-dragging': dragging }" :style="{ width: pct + '%' }">
      ...
      <div ref="trackRef" class="tl-field-track" @mousedown="startDrag" />
      ...
      <input
        ...
        @mousedown.stop
      />
      <span v-else class="tl-field-value" @mousedown.stop @click="beginEdit">{{ display }}</span>
    </div>

    <template v-if="tokens.length">
      <button type="button" class="tl-field-chevron" @click="menuOpen = !menuOpen">
...
.tl-field-wrap { position: relative; display: flex; align-items: center; gap: 6px; }
.tl-field {
  position: relative; flex: 1; min-width: 0; height: 32px; border-radius: 8px;
  background: rgba(238, 238, 239, 0.51);
  transition: box-shadow 0.12s ease, background 0.12s ease;
}
.tl-field:hover:not(.is-editing) { background: rgba(170, 170, 170, 0.2); }
.tl-field.is-editing {
  box-shadow: inset 0 0 0 1px rgba(0, 0, 0, 0.14), inset 0 0 0 0.5px rgba(255, 255, 255, 0.06);
}
.tl-field-fill {
  position: absolute; left: 0; top: 0; bottom: 0; min-width: 32px; border-radius: 8px;
...
.scrubber-ticks { position: absolute; inset: 0; pointer-events: none; z-index: 1; }
.scrubber-tick {
...
.tl-field-label {
  ...
  font-size: 13px; font-weight: 500; color: #585858; pointer-events: none; z-index: 3;
}
.tl-field-value {
  position: absolute; right: 12px; top: 50%; transform: translateY(-50%);
  font-size: 13px; font-weight: 400; color: #17181c; cursor: text; z-index: 3;
  font-variant-numeric: tabular-nums;
}
.tl-field-input {
  position: absolute; right: 8px; top: 50%; transform: translateY(-50%); width: 64px;
  text-align: right; border: none; background: transparent; font: inherit;
  font-size: 13px; font-weight: 400; color: #17181c; outline: none; z-index: 4;
  font-variant-numeric: tabular-nums;
}
.tl-field-thumb {
  position: absolute; right: 8px; top: 7px; height: 18px; width: 2px;
  background: #767676; border-radius: 20px; opacity: 0; pointer-events: none;
...
.tl-field-chevron {
  ...
  background: rgba(238, 238, 239, 0.51); color: #8b9099; cursor: pointer;
  transition: background 0.12s ease, scale 0.12s ease;
}
.tl-field-chevron:hover { background: rgba(170, 170, 170, 0.2); }
...
.tl-field-menu-item {
  display: flex; width: 100%; align-items: center; gap: 6px; border: none;
  background: transparent; border-radius: 6px; padding: 6px 8px;
  font-size: 13px; color: #17181c; cursor: pointer; text-align: left;
}
.tl-field-menu-item:hover { background: rgba(170, 170, 170, 0.2); }
.tl-field-menu-dim { color: #8b8b8b; }

</style>

<style>
/* dark overrides — unscoped so the `.dark` ancestor class matches */
.dark .tl-field { background: rgba(255, 255, 255, 0.04); }
.dark .tl-field:hover:not(.is-editing) { background: rgba(255, 255, 255, 0.06); }
.dark .tl-field.is-editing {
  box-shadow: inset 0 0 0 1px rgba(255, 255, 255, 0.14), inset 0 0 0 0.5px rgba(255, 255, 255, 0.06);
}
.dark .tl-field-fill {
...
.dark .tl-field-fill.is-dragging { background: rgba(255, 255, 255, 0.12); }
.dark .tl-field-label { color: #979797; }
.dark .tl-field-value,
.dark .tl-field-input { color: #fbfbfb; }
.dark .scrubber-tick { background: rgba(255, 255, 255, 0.1); }
.dark .tl-field-chevron { background: rgba(255, 255, 255, 0.04); }
.dark .tl-field-chevron:hover { background: rgba(255, 255, 255, 0.06); }
.dark .tl-field-menu {
  background: #232323;
  box-shadow: 0 0 0 1px rgba(255, 255, 255, 0.08), 0 8px 24px rgba(0, 0, 0, 0.4);
}
.dark .tl-field-menu-item { color: #fbfbfb; }
.dark .tl-field-menu-item:hover { background: rgba(255, 255, 255, 0.06); }
</style>

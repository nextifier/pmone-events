...
export function rrPerim(w: number, h: number, r: number): number {
  ...
  return (
    2 * Math.max(0, w - 2 * rr) + 2 * Math.max(0, h - 2 * rr) + 2 * Math.PI * rr
  );
}

export function shapePerim(
  w: number,
  h: number,
  r: number,
  kind: "pill" | "circle",
  ...
  if (kind === "circle")
    return 2 * Math.PI * Math.max(0, Math.min(r, Math.min(w, h) / 2));

...
export function injectGlow(
  ...
  const ratio =
    shapePerim(opts.width, opts.height, opts.cornerRadius, opts.kind) /
    rrPerim(REF_W, REF_H, REF_R);
...
export function updateGlow(
  ...
  if (!h.relocTween || h.relocTween.done) {
    if (h.appearedAt === 0) {
      ...
      const fadeInTarget =
        BASE_OP + (PEAK_OP - BASE_OP) * smoothstep(LO, HI, nl);
  ...
  h.extraGroup.style.opacity = Math.min(
    1,
    h.glowOpacity * EXTRA_INTENSITY * m,
  ).toFixed(3);
}
...
export function resizeGlow(
  ...
  for (const svg of Array.from(
    container.querySelectorAll(".metal-fx-glow-svg"),
  )) {

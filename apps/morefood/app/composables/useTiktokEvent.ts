export function useTiktokEvent() {
  ...
  // The SAME pixel id(s) analytics.client.ts loads, so the server-side Events
  // API (`pixel_code`) fires to whichever pixel the client pixel was booted
  // with and the two never split.
  const pixelIds = computed(() =>
    ([] as string[]).concat(appConfig.settings.tiktokPixelId).filter(Boolean),
  );
